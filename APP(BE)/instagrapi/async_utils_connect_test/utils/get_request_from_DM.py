...
#     # 검사 대상을 최대 3개의 Post로 한정 짓는다.
#     j = 0
#     for idx in range(raw_posts_len):
#         post_id = raw_user_posts[idx].id
#         print(post_id)
#         post_likers = cl.media_likers(post_id) # 해당 post의 likes : post_likers
#         if len(post_likers) == 0: # 좋아요 누른 사람이 없다면 -> 검사도 안했다는 뜻
#             user_posts.append(raw_user_posts[idx])
#         else:
#             # if 49617754574 not in post_likers:
#             #     user_posts.append(raw_user_posts[idx])
#             #     j +=1
#             for n in range(len(post_likers)):
#                 print(post_likers[n].pk)
...
#     print(f' Raw User Post에서 user_posts 추출 완료')

#     print(f'Target Posts Length : {len(user_posts)}')

#     await download_media(cl,user_posts)
